...
class Sync {
	...
	getPages(next = '', expunged = false, seekdate = "") {
		return new Promise((resolve, reject) => {
			try {
				const request = https.request({
					method: 'GET',
					hostname: this.host,
					path: `/?next=${next}&f_cats=0&advsearch=1&f_sname=on&f_stags=on&f_sh=${expunged ? 'on' : ''}&f_spf=&f_spt=&f_sfl=on&f_sfu=on&f_sft=on&f_search=language:english%24&seek=${seekdate}`,
					headers: {
	...
	async run() {
		...
		connection.connect(async (err) => {
			...
			const getPages = async (expunged) => {
				...
				let next = '';
				//2023-03-04
				let skiptodate = "2023-05-30"
				while (!finish) {
					console.log(skiptodate)
					await this.sleep(1);
					console.log(`requesting ${expunged ? 'expunged' : 'default'} page ${page}...`);
					const curList = await this.retryResolver(() => this.getPages(next, expunged,skiptodate), this.retryTimes);
